...
#include "Annapureddy_Shreyash_1072336621_Q3EC1.h"
...
int main(void)
{
	// Creating an PGM image object.
	std::string fileName = "sub_fractal_1.pgm";		//Desired file name of image
	 ...
	 * 4 = Mini mandelbrot */
	int fractalDesign = 1;
	double boundingBox[] = {-2.0,1.0,-1.0,1.0};		// Bounding values for a mandelbrot
	desiredFractal(boundingBox,fractalDesign);		// Determine bounding values for fractal design
	
	
	/* To view a clear image of the pattern we determine a new xRes and yRes 
	 *  based on the aspect ratio determined from the boundingBox.
	 * The following steps are OPTIONAL */
	int newDim[] = {0,0};
	int imgCol = 0;		// Index definition for newDim[]
	int imgRow = 1;		// Index definition for newDim[]
	aspectRatio(boundingBox,newDim);
	mandelbrot->changeResolution(newDim[imgCol], newDim[imgRow]);	// Change the image resolution
...
void PGMImage::checkFileName() {
	...
	if(wrongExtension) {
		validFile = false;
		std::cerr << "Error: file extension either incorrect, does not exist or incorrect case. expected: *.pgm\n";
	}
}
